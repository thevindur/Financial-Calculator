...
public class CalSimpleInterest {
    ...
    public static void savings(Stage primaryStage) {
        Pane root = new Pane();
        
        Label lblHome = new Label("Simple Interest");
...
//Creating the labels and texts required for the third grid pane.
        Label lblSavings = new Label(" Results of the Simple Interest Calculator.");
        lblSavings.setId("lblResult");
        Label lblInvested = new Label(" Amount invested by the user : ");
...
//Initialising the labels and the texts to the third grid pane.
        gridPane3.add(lblSavings,0,1,2,1);
...
//Setting up button actions for each button on the number pad.
        buttonCal.setOnAction( e-> {
            ...
            try{
                if(txt3.getText().isEmpty()){
                    double investedAmt = Double.parseDouble(myString.trim()) ;
                    ...
                    double totInterest = investedAmt * interest * noOfYrs;
                    double totAmt = investedAmt + totInterest;

                    txtInvested.setText("Rs. "+myString);
                    txtInt.setText(myString2+"%");
                    txtYears.setText(myString1);
                    txtTotInt.setText("Rs. "+Double.toString(totInterest));
                    txtTotal.setText("Rs. "+Double.toString(totAmt));
                    txt3.setText("Rs. "+Double.toString(totAmt));
                }
                else if(txt.getText().isEmpty()){
                    double noOfYrs = Double.parseDouble(myString1.trim());
                    double intRate = Double.parseDouble(myString2.trim());
                    double finalAmt = Double.parseDouble(myString3.trim());
                    double interest = intRate/100;
                    double investedAmt = finalAmt/(1 + (interest * noOfYrs));

                    txtInvested.setText("Rs. "+Double.toString(investedAmt));
                    txt.setText("Rs. "+Double.toString(investedAmt));
                    ...
                    double finalAmt = Double.parseDouble(myString3.trim());
                    double interest = intRate/100;
                    double noOfYrs = (finalAmt-investedAmt)/(investedAmt * interest);
                    ...
                    double noOfYrs = Double.parseDouble(myString1.trim());
                    double finalAmt = Double.parseDouble(myString3.trim());
                    double interest = (finalAmt-investedAmt)/(investedAmt * noOfYrs);
